...
class MaterialAssemblyProvider with ChangeNotifier {
  ...
  static const String compFeature = "materialAssemblyComparison";
  ...
  Future<Map<String, dynamic>> getByIdMaterialAssembly() async {
    http.StreamedResponse response = await networkService
        .post("/get-material-assembly/", {"matno": editController.text});
  ...
  Future<http.StreamedResponse> processFormInfo() async {
    http.StreamedResponse response = await networkService.post(
        "/add-material-assembly/", GlobalVariables.requestBody[featureName]);
    return response;
  }
  ...
  Future<http.StreamedResponse> addMaterialAssemblyDetails(
    ...
    var payload = manualOrder
        ? matAssembly
        : GlobalVariables.requestBody['MaterialAssemblyDetails'];
  ...
  Future<http.StreamedResponse> addMaterialAssemblyProcessing(
    ...
    var payload = manualOrder
        ? matAssembly
        : GlobalVariables.requestBody['MaterialAssemblyProcessing'];

    http.StreamedResponse response = await networkService.post(
        "/add-material-assembly-processing/", payload);
    return response;
  }
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri =
                      Uri.parse("${NetworkService.baseUrl}${data['pic']}");
  ...
  void initComparisonReport() async {
    String jsonData =
    ...
    GlobalVariables.requestBody[compFeature] = {};
    formFieldDetails.clear();
    widgetList.clear();
    ...
    List<Widget> widgets =
        await formService.generateDynamicForm(formFieldDetails, compFeature);
    widgetList.addAll(widgets);
    notifyListeners();
  }

  void getMatAssemblyComparison() async {
    matComp.clear();
    http.StreamedResponse response = await networkService.post(
        "/mat-assembly-comparison/", GlobalVariables.requestBody[compFeature]);
    if (response.statusCode == 200) {
      matComp = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
}
