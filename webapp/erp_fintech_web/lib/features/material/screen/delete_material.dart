...
class DeleteMaterial extends StatefulWidget {
  static String routeName = 'DeleteMaterial';
  const DeleteMaterial({super.key});

  @override
  State<DeleteMaterial> createState() => _DeleteMaterialState();
}

class _DeleteMaterialState extends State<DeleteMaterial> {
  @override
  void initState() {
    super.initState();
    ...
    return Consumer<MaterialProvider>(builder: (context, provider, child) {
      return Scaffold(
        appBar: PreferredSize(
            ...
            child: const CommonAppbar(title: "Delete Material")),
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          Map<String, dynamic> resp =
                              await provider.getByIdMaterial();
                          if (resp != {}) {
  ...
  void _showTablePopup(BuildContext context, Map<String, dynamic> material) {
    showDialog(
      context: context,
              ...
              child: Column(
                ...
                children: [
                  DataTable(
                    ...
                    rows: [
                      DataRow(cells: [
                        DataCell(Text("${material['matno']}")),
                        DataCell(Text("${material['matDescription']}")),
                        DataCell(Text("${material['hsnCode']}")),
                        DataCell(Text("${material['mrp']}")),
                        DataCell(Text("${material['listPrice']}")),
                        DataCell(Text("${material['mst']}")),
                          ...
                          onPressed: () async {
                            ...
                            if (confirmation) {
                              ...
                              http.StreamedResponse response =
                                  await networkService.post("/delete-material/",
                                      {"matno": '${material['matno']}'});
                              if (response.statusCode == 204) {
                                context.pushReplacementNamed(
                                    DeleteMaterial.routeName);
