...
class BpProcessingProvider with ChangeNotifier {
  ...
  void initReport() async {
    ...
    String jsonData =
        '[{"id":"bpCode","name":"Business Partner","isMandatory":true,"inputType":"dropdown", "dropdownMenuItem" : "/get-bp-ob/"}]';
  ...
  void getReport() async {
    bppReport.clear();
    http.StreamedResponse response = await networkService.post(
        "/bp-processing-report/", GlobalVariables.requestBody[reportFeature]);
    if (response.statusCode == 200) {
      bppReport = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
