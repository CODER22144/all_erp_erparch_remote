...
class DbnoteProvider with ChangeNotifier {
  ...
  List<SearchableDropdownMenuItem<String>> yesno = [];
  ...
  void setDropdowns() async {
    ...
    yesno = await formService.getDropdownMenuItem("/get-yesno/");
    notifyListeners();
  }
  ...
  Future<http.StreamedResponse> processFormInfo(
    ...
    for (int i = 0; i < tableRows.length; i++) {
      inwardDetails.add({
        ...
        "unit": tableRows[i][19],
        "vtype": tableRows[i][20]  == "" ? null : tableRows[i][20],
        "wdocno": tableRows[i][21] == "" ? null : tableRows[i][21],
        "stockLess": tableRows[i][22] == "" ? "N" : tableRows[i][22],
      });
    }
