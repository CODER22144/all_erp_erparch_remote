...
class BpBreakupProvider with ChangeNotifier {
  ...
  static const String reportFeature = "BpBreakupSummary";
  ...
  void initReport() async {
    ...
    String jsonData =
        '[{"id":"bpCode","name":"Business Partner Code","isMandatory":true,"inputType":"dropdown","dropdownMenuItem":"/get-bp-ob/"},{"id":"fmatno","name":"From Material No.","isMandatory":false,"inputType":"text","maxCharacter":15},{"id":"tmatno","name":"To Material No.","isMandatory":false,"inputType":"text","maxCharacter":15}]';
  ...
  void getBreakupSummary() async {
    bpBreakupSummary.clear();
    http.StreamedResponse response = await networkService.post(
        "/get-bp-breakup-summary/", GlobalVariables.requestBody[reportFeature]);
    if (response.statusCode == 200) {
      bpBreakupSummary = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
}
