...
class ProductBreakupProvider with ChangeNotifier {
  ...
  static const String costCompFeature = "pbCostComparsionReport";
  ...
  String getRightUrl(String path) {
  ...
  void pbCostingTable() async {
    ...
    for (int i = 0; i < pbCostingReport.length; i++) {
      ...
      columns = const [
        ...
        DataColumn(
            label: Text("Doc", style: TextStyle(fontWeight: FontWeight.bold))),
        DataColumn(
            label: Text("Pack", style: TextStyle(fontWeight: FontWeight.bold))),
        DataColumn(
            label:
      ...
      rows.addAll([
        DataRow(cells: [
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['images']}"));
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['cp']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Text("P.COST: ${data['tamount'] ?? '0.00'}")),
        ...
        DataRow(cells: [
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['drawing']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['fmea']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['pfd']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Text("PC: ${data['pramount'] ?? '0.00'}")),
        ...
        DataRow(cells: [
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri =
                      Uri.parse(getRightUrl("${data['asdrawing']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['ccr']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Text("FC: ${data['processing'] ?? '0.00'}")),
          DataCell(Text("OH: ${data['overheadamount'] ?? '0.00'}")),
          const DataCell(Text("")),
          DataCell(Text("TP: ${data['targetp'] ?? ''}")),
          const DataCell(SizedBox()),
        ]),
        DataRow(cells: [
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri =
                      Uri.parse(getRightUrl("${data['vendrawing']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
          ...
          DataCell(Row(children: [
            ...
            Visibility(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  final Uri uri = Uri.parse(getRightUrl("${data['qcformat']}"));
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.inAppBrowserView);
                  } else {
                    throw 'Could not launch';
  ...
  void getPbCostingRateDetails(String matno) async {
    pbCostingMatSourceReport.clear();
    http.StreamedResponse response =
        await networkService.post("/pb-cost-mat-source/", {"matno": matno});
    if (response.statusCode == 200) {
      pbCostingMatSourceReport =
          jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }

  void initCostComparisonReport() async {
    String jsonData =
        '[{"id":"csId","name":"Cost Status","isMandatory":true,"inputType":"dropdown","dropdownMenuItem":"/get-cost-status/"},{"id":"fmatno","name":"From Material No.","isMandatory":false,"inputType":"text","maxCharacter":15},{"id":"tmatno","name":"To Material No.","isMandatory":false,"inputType":"text","maxCharacter":15}]';
    GlobalVariables.requestBody[costCompFeature] = {};
    formFieldDetails.clear();
    reportWidgetList.clear();
    ...
    List<Widget> widgets =
        await formService.generateDynamicForm(formFieldDetails, costCompFeature);
    reportWidgetList.addAll(widgets);
    notifyListeners();
  }

  void getPbCostComparison() async {
    pbCostComp.clear();
    http.StreamedResponse response = await networkService.post(
        "/pb-cost-comparison/", GlobalVariables.requestBody[costCompFeature]);
    if (response.statusCode == 200) {
      pbCostComp = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
}
