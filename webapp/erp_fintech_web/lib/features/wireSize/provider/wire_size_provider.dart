...
class WireSizeProvider with ChangeNotifier {
  ...
  static const String assemblyFeature = "wireSizeAssemblyReport";
  static const String materialFeature = "wireSizeAssemblyMaterialReport";
  ...
  List<dynamic> wsMatReport = [];
  List<dynamic> wsAssReport = [];
  ...
  Future<http.StreamedResponse> addWireSizeDetailsOnly(
    ...
    for (int i = 0; i < tableRows.length; i++) {
      wireSizeDetails.add({
        ...
        "assno": tableRows[i][15] == "" ? null : tableRows[i][15],
        "remark": tableRows[i][16] == "" ? null : tableRows[i][16],
        "partType": tableRows[i][17] == "" ? null : tableRows[i][17]
      });
    }
  ...
  void wireSizeAssemblyReportWidget() async {
    String jsonData =
        '[{"id":"matno","name":"Material No.","isMandatory":true,"inputType":"text","maxCharacter":15},{"id":"assno","name":"Assembly No.","isMandatory":true,"inputType":"text","maxCharacter":15}, {"id" : "repId", "name" : "With/Without TL (W/O)", "isMandatory" : false, "inputType" : "text", "maxCharacter" : 1}]';
    GlobalVariables.requestBody[assemblyFeature] = {};
    formFieldDetails.clear();
    widgetList.clear();
    ...
    List<Widget> widgets = await formService.generateDynamicForm(
        formFieldDetails, assemblyFeature);
    widgetList.addAll(widgets);
    notifyListeners();
  }

  void wireSizeAssemblyMaterialReportWidget() async {
    String jsonData =
        '[{"id":"matno","name":"Material No.","isMandatory":true,"inputType":"text","maxCharacter":15},{"id":"assno","name":"Assembly No.","isMandatory":false,"inputType":"text","maxCharacter":15}]';
    GlobalVariables.requestBody[materialFeature] = {};
    formFieldDetails.clear();
    widgetList.clear();
    ...
    List<Widget> widgets = await formService.generateDynamicForm(
        formFieldDetails, materialFeature);
    widgetList.addAll(widgets);
    notifyListeners();
  }

  void getWsMaterialRep() async {
    wsMatReport.clear();
    http.StreamedResponse response = await networkService.post(
        "/wire-size-mat-rep/", GlobalVariables.requestBody[materialFeature]);
    if (response.statusCode == 200) {
      wsMatReport = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
}
