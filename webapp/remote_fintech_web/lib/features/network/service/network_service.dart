...
class NetworkService {
  static const String baseUrl = "http://erpapiarch.rcinz.com";
  ...
  Future<http.StreamedResponse> authorizeGst(
      dynamic requestBody, String url) async {
    ...
    var request = http.Request('GET',
        Uri.parse('$url/einvoice/authenticate?email=${requestBody['email']}'));
    request.bodyFields = {};
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
  ...
  void logError(dynamic errorMap) async {
    ...
    request.body = jsonEncode({
      ...
      "user_id": loginId,
      "api_payload": errorMap['featureName'] != null
          ? GlobalVariables.requestBody[errorMap['featureName']]
          : errorMap['payload']
    });
    request.headers.addAll(headers);
