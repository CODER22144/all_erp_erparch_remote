...
class CrNoteProvider with ChangeNotifier {
  ...
  static const String masterDetailFeatureName = "SaleCrnoteItemDetails";
  ...
  Future<http.StreamedResponse> processFormInfo(
    ...
    if (manual) {
      ...
      GlobalVariables.requestBody[featureName]['SaleCrnoteItemDetails'] = inwardDetails;
      payload = [GlobalVariables.requestBody[featureName]];
    } else {
      payload = GlobalVariables.requestBody[featureName];
    }
    http.StreamedResponse response = await networkService.post(
        "/add-credit-note/", payload);
    return response;
  }
  ...
  void getDbNoteReport() async {
    report.clear();
    http.StreamedResponse response = await networkService.post(
        "/cr-note-report/", GlobalVariables.requestBody[reportFeature]);
  ...
  void getCrNotePostPending() async {
    ...
    http.StreamedResponse response = await networkService.get("/get-crnote-pending/");
    if(response.statusCode == 200) {
