...
class DeleteBpBreakupDetails extends StatefulWidget {
  static String routeName = 'DeleteBpBreakupDetails';
  const DeleteBpBreakupDetails({super.key});

  @override
  State<DeleteBpBreakupDetails> createState() => _DeleteBpBreakupDetailsState();
}

class _DeleteBpBreakupDetailsState extends State<DeleteBpBreakupDetails> {
        ...
        builder: (context, provider, child) {
          return Scaffold(
            appBar: PreferredSize(
                ...
                child: const CommonAppbar(title: "Delete Business Partner Breakup")),
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () async {
                                ...
                                http.StreamedResponse response = await networkService.post("/get-bp-breakup-details/" , {"bpCode" : provider.editController.text, "pId" : provider.editController2.text});
  ...
  void _showTablePopup(BuildContext context, List<dynamic> orderBalance) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Delete Business Partner Breakup Details',
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            if(confirmation) {
                              NetworkService networkService = NetworkService();
                              http.StreamedResponse response = await networkService.post("/delete-bp-breakup-details/", {"bpbdId" : '${data['bpbdId']}'});
                              if(response.statusCode == 200) {
                                context.pushReplacementNamed(DeleteBpBreakupDetails.routeName);
                              } else {
                                var message = jsonDecode(await response.stream.bytesToString());
                                networkService.logError({
                                  "errorCode" : message['errorCode'] ?? "",
                                  "errorMsg" : message['message'] ?? "",
                                  "endpoint" : "/delete-bp-breakup-details/",
                                  "featureName" : null,
                                  "payload" : {"bpbdId" : '${data['bpbdId']}'}
                                });
