...
class SalesDebitNoteProvider with ChangeNotifier {
  ...
  static const String masterDetailFeatureName = "SaleDbnoteItemDetails";
  ...
  Future<http.StreamedResponse> processFormInfo(
    ...
    if (manual) {
      for (int i = 0; i < tableRows.length; i++) {
        ...
        GlobalVariables.requestBody[featureName]['SaleDbnoteItemDetails'] = inwardDetails;
      payload = [GlobalVariables.requestBody[featureName]];
    } else {
      payload = GlobalVariables.requestBody[featureName];
    }
    http.StreamedResponse response =
        await networkService.post("/add-sale-debit-note/", payload);
    return response;
  }
