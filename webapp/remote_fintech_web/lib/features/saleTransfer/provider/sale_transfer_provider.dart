...
class SaleTransferProvider with ChangeNotifier {
  ...
  dynamic paymentPending;

  List<DataRow> payRows = [];
  ...
  void initPaymentPendingReport() async {
    ...
    reportWidgetList.clear();
    String jsonData =
        '[{"id":"lcode","name":"Party Code","isMandatory":false,"inputType":"dropdown","dropdownMenuItem":"/get-ledger-codes/"}, {"id" : "stcode","name" : "State", "isMandatory": false, "inputType" : "dropdown", "dropdownMenuItem" : "/get-states/"},{"id":"iday","name":"Days","isMandatory":false,"inputType":"number","default":0}]';
  ...
  void getPaymentPendingReport() async {
    paymentPending = {};
    http.StreamedResponse response = await networkService.post(
        "/payment-pending/",
        GlobalVariables.requestBody[reportFeature]);
  ...
  void getBillRows() {
    ...
    for(var data in paymentPending['LcodeData']) {
      payRows.add(DataRow(cells: [
        DataCell(Text('${data['lcode']}', style: const TextStyle(fontWeight: FontWeight.w500))),
        DataCell(Text('${data['lname']}', style: const TextStyle(fontWeight: FontWeight.w500))),
        DataCell(Text('${data['city']}', style: const TextStyle(fontWeight: FontWeight.w500))),
        DataCell(Text('${data['stateName']}', style: const TextStyle(fontWeight: FontWeight.w500))),
      ...
      for(var details in data['details']) {
        payRows.add(DataRow(cells: [
          DataCell(Text('${details['transId']}')),
          DataCell(Text('${details['billNo']}')),
          DataCell(Text('${details['dbillDate']}')),
          DataCell(Text('${details['vtype']}')),
          DataCell(Text('${details['tdays']}')),
          DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${details['tamount']}')))),
          DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${details['paid']}')))),
          DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${details['bamount']}')))),
          DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${details['rowBamount']}')))),
        ]));
      }

      var sumarry = jsonDecode(data['summary']);
      ...
      payRows.add(DataRow(cells: [
        ...
        const DataCell(Text('')),
        const DataCell(Text('')),
        DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${sumarry['totalPaid']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
        DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${sumarry['totalBamount']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
        DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${sumarry['totalPending']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
      ]));
      payRows.add(emptyRow);
    }
    var grandTotal = jsonDecode(paymentPending['GrandSummary']);
    ...
    payRows.add(DataRow(cells: [
      ...
      const DataCell(Text('')),
      const DataCell(Text('')),
      DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${grandTotal['grandTotalPaid']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
      DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${grandTotal['grandTotalBamount']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
      DataCell(Align(alignment: Alignment.centerRight,child: Text(parseDoubleUpto2Decimal('${grandTotal['grandTotalPending']}'), style: TextStyle(fontWeight: FontWeight.bold)))),
    ]));
