...
class BankProvider with ChangeNotifier {
  ...
  List<dynamic> unClaimedPayment = [];
  List<dynamic> claimedPayment = [];
  ...
  void getUnclaimedPayments() async {
    unClaimedPayment.clear();
    http.StreamedResponse response = await networkService.get("/unclaimed-payments/");
    if (response.statusCode == 200) {
      unClaimedPayment = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }

  void getClaimedPayments() async {
    claimedPayment.clear();
    http.StreamedResponse response = await networkService.get("/claimed-payments/");
    if (response.statusCode == 200) {
      claimedPayment = jsonDecode(await response.stream.bytesToString());
    }
    notifyListeners();
  }
  ...
  void initReport() async {
    ...
    String jsonData =
        '[{"id":"lcode","name":"Party Code","isMandatory":false,"inputType":"dropdown","dropdownMenuItem":"/get-ledger-codes/"},{"id":"type","name":"Trans. Type (D/W)","isMandatory":false,"inputType":"text","maxCharacter":1, "default" : "D"},{"id":"fromDate","name":"From Date","isMandatory":true,"inputType":"datetime"},{"id":"toDate","name":"To Date","isMandatory":true,"inputType":"datetime"}]';
  ...
  void getRows(BuildContext context) {
    ...
    for (var data in bankStatement) {
      ...
      rows.add(DataRow(cells: [
        ...
        DataCell(SizedBox(width: 400,child: Text('${data['transDescription'] ?? "-"}', maxLines: 2))),
        DataCell(Text('${data['refNumber'] ?? "-"}')),
        DataCell(Text('${data['dvalueDate'] ?? "-"}')),
        DataCell(Align(
  ...
  Future<http.StreamedResponse> processFormInfo() async {
    http.StreamedResponse response = await networkService.post(
        "/claim-payment/", GlobalVariables.requestBody[featureName]);
    return response;
  }
  ...
  Future<http.StreamedResponse> postClaimedPayments() async {
    http.StreamedResponse response = await networkService.post(
        "/post-claimed-payments/", {});
    return response;
  }
}
