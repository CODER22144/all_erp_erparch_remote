...
class AdditionalOrder extends StatefulWidget {
  static String routeName = "/additionalOrder";
...
class _AdditionalOrderState extends State<AdditionalOrder> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<AdditionalOrderProvider>(
        builder: (context, provider, child) {
      return Scaffold(
        appBar: PreferredSize(
            preferredSize:
                Size.fromHeight(MediaQuery.of(context).size.height * 0.07),
            child: const CommonAppbar(title: 'Additional Purchase Order')),
                            ...
                            if (formKey.currentState!.validate()) {
                              ...
                              if (confirmation) {
                                ...
                                if (result.statusCode == 200) {
                                  context.pushReplacementNamed(AdditionalOrder.routeName);
                                } else if (result.statusCode == 400) {
                                  networkService.logError({
                                    "errorCode" : message['errorCode'] ?? "",
                                    "errorMsg" : message['message'] ?? "",
                                    "endpoint" : "/additional-purchase-order/",
                                    "featureName" : AdditionalOrderProvider.featureName
                                  });
