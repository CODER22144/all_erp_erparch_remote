...
from cafintech_api.serializers.product_breakup_serializer import ProductBreakupCostComparisonSerializer, ProductBreakupSerializer
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def productBreakupCostComparison(request):
    try:
        serializer = ProductBreakupCostComparisonSerializer(data=request.data)
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [cost].[ProductBreakupCostComparision] %s",(json.dumps(serializer.data),))
            json_data = ConvertToJson(cursor)
            return JsonResponse(json_data, safe=False)
        UNSUCCESSFUL_REQUEST['message'] = serializer.errors
        return Response(UNSUCCESSFUL_REQUEST, status=400)
