...
from cafintech_api.serializers.bp_breakup_serializer import BpBreakupSerializer, BpBreakupSummarySerializer
...
def getBpBreakupReport(request):
    try:        
        ...
        cursor.close()
        return render(request, "breakup_rep.html", context)
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def getBpBreakupSummary(request):
    try:
        serializer = BpBreakupSummarySerializer(data=request.data)
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [cost].[BPBreakupSummary] %s",(json.dumps(serializer.data),))
            json_data = [data[0] for data in cursor.fetchall()]
            json_data = "".join(json_data)
            cursor.close()
