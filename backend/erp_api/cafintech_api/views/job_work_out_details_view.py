...
from cafintech_api.serializers.job_work_out_serializer import JobWorkOutClearPendingSerializer, JobWorkOutClearReportSerializer, JobWorkOutClearSerializer, JobWorkOutSerializer
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def createJobWorkOutClear(request):
    try:
        orderSerializer = JobWorkOutClearSerializer(data=request.data, many=True)
        if(orderSerializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [inven].[uspAddJobWorkOutClear] %s",(json.dumps(orderSerializer.data),))
            cursor.close()
            return Response(orderSerializer.data)
        errors = {}
        if not orderSerializer.is_valid():
            errors["JobWorkOut"] = orderSerializer.errors
        UNSUCCESSFUL_REQUEST['message'] = errors
        return Response(UNSUCCESSFUL_REQUEST, status=400)
    except Exception as e:
        return Response(data=generate_error_message(e), status=500, exception=e)
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def deleteJobWorkOutClear(request):
    try:
        cursor = connections[request.user.cid.cid].cursor()
        cursor.execute(f"exec [inven].[uspDeleteJobWorkOutClear] %s", (request.data['clId'], ))
        cursor.close()
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def getJobWorkOutClearReport(request):
    try:
        serializer = JobWorkOutClearReportSerializer(data=request.data)
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [inven].[uspGetJobWorkOutClear] %s",(json.dumps(serializer.data),))
            json_data = ConvertToJson(cursor)
            cursor.close()
            return JsonResponse(json_data, safe=False)
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def getJobWorkOutClearPending(request):
    try:
        serializer = JobWorkOutClearPendingSerializer(data=request.data)
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [inven].[uspJobWorkOutClear] %s",(json.dumps(serializer.data),))
