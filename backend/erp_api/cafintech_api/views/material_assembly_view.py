...
def getProductBreakup(request, matno, cid):
    try:
        ...
        image = json_data[0]['images']
        if image:
            if not image.startswith('http'):
                image = "http://erpapi.rcinz.com"+image
        else:
            image = "http://erpapi.rcinz.com/static/images/no-image.png"
...
def getPartAssembly(request, matno, cid):
    try:
        ...
        context = {
            "mat" : json_data[0] if len(json_data) > 0 else {},
        }
